...
export default class CourseComponent implements OnInit {
    ...
    const observer = {
      next: (res: Course[]) => {
        ...
        this.courses.forEach((course) => {
          this.teacherService
            .loadImage(course.teacher.images[0].imageId)
            .subscribe({
              next: (res: Image) => {
  ...
  public updateCourse(_course: Course): void {
    ...
    this.courseService
      .updateCourse(_course.courseId, _course, this.course)
      .subscribe(observer);
  }
  ...
  public search(form: NgForm): void {
    let filteredCourses = this.allCourses.filter((course) =>
      course.courseName
        .toLowerCase()
        .includes(form.value.courseName.toLowerCase())
    );

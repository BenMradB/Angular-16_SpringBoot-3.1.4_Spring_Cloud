...
export class TeacherComponent implements OnInit {
    ...
    if (this.isImageUpdated) {
      ...
      this.teacherService
        ...
        .subscribe({
          next: (res: Image) => {
            this.teacherService
              .updateTeacher(teacher.teacherId, _teacher, res.imageId)
              .subscribe({
                next: (res: Teacher) => {
                  this.getTeachers();
      ...
      this.teacherService
        .updateTeacher(teacher.teacherId, _teacher, this.teacher?.image.imageId)
        .subscribe({
          next: (res: Teacher) => {
            this.getTeachers();
            document.getElementById('updateClose')?.click();
  ...
  public search(form: NgForm): void {
    let filteredTeachers = this.allTeachers.filter((teacher) =>
      teacher.userName.toLowerCase().includes(form.value.userName.toLowerCase())
    );
    ...
    this.teacherService
      ...
      .subscribe({
        next: (res: Image) => {
          ...
          this.teacher!.imageStr = `data:${
            this.teacher!.images[0].imageType
          };base64,${this.teacher!.images[0].image}`;
        },
        error: (error: HttpErrorResponse) => {},
      });
  }

...
export class UserComponent implements OnInit {
  ...
  public grantRoleToUser(userId: number, form: NgForm): void {
    const roleToGrant = form.value.role;
    const userToGrantRoleTo = this.users.filter(
      (user) => user.user_id === userId
    )[0];
    const haveRole = userToGrantRoleTo.roles.filter(
      (roleObj) => roleObj.role_id === +roleToGrant
    );
    ...
    if (!haveRole.length) {
      ...
      window.alert(`This user have already this ${haveRole[0].role} role. `);
    }
  }
  ...
  public search(form: NgForm): void {
    let filteredUsers = this.allUsers.filter((user) =>
      user.username.toLowerCase().includes(form.value.username.toLowerCase())
    );
  ...
  public revokeRoleFromUser(userId: number, role: Role): void {
    ...
    if (conf) {
      this.userService
        .revokeRoleFromUser(userId, role.role_id)
        .subscribe(() => {

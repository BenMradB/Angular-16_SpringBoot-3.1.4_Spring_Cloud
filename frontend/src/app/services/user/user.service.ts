...
export class UserService {
  ...
  public getUserByEmail(email: string): Observable<User> {
    return this.http.get<User>(`${this.apiServerUrl}/get/email/${email}`);
  }
  ...
  public grantRoleToUser(userId: number, roleId: number): Observable<User> {
    return this.http.post<User>(
      `${this.apiServerUrl}/grant/role/${roleId}/user/${userId}`,
      null
    );
  }
  ...
  public revokeRoleFromUser(userId: number, roleId: number): Observable<void> {
    return this.http.post<void>(
      `${this.apiServerUrl}/revoke/role/${roleId}/user/${userId}`,
      null
    );
  }
